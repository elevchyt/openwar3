...
export interface FxSnapshot {
  /** One-shot effect models (`drainSpellEffects`): follow `targetId`'s record when > 0, or
   *  ride one of its BONES when the effect names an `attach`. See `SimSpellEffect` — the
   *  shape is the sim's, spelled out there rather than restated here. */
  effects: SimSpellEffect[];
