...
export interface MapFormatProfile {
   ...
   *  most such maps play. */
  laterFormat: boolean;
  /** What the `Blz…` natives count weapons AND ability levels from: 0 for a map saved by a
   *  1.31+ editor, 1 otherwise. "In 1.30 or lower, the function is 1-indexed, but in 1.31 and
   *  newer, it is 0-indexed" (hiveworkshop 319334; levels moved in the same patch, 316163) — and a map a 1.31+ editor wrote only ever ran on a 1.31+
...
export function readMapFormat(mpq: MpqDataSource): MapFormatProfile {
  ...
  return {
    ...
    blzIndexBase: editorBuild >= 131 ? 0 : 1,
  };
}
