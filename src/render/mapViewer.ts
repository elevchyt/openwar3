...
// THREE things have to agree on it: the `cursor:` rule, the tinted hover hand's (overlayCursor)
...
export class MapViewerScene {
  ...
  /** Ctrl+C's camera lock: the sim id of the unit the camera is riding, or null. Unlike the
  ...
  loadMap(bytes: Uint8Array): void {
    ...
    // Every follow goes with the world it was riding — a sim id from the last match names a
    // different unit in this one.
  ...
  private localViewHooks(): Partial<EngineHooks> {
    return {
      ...
      setCameraTargetUnit: (id, xOff, yOff) => {
        ...
        this.releaseCameraRide(); // …and Ctrl+C's, for the same reason
  ...
  /** The spring itself, shared by the held follow above and Ctrl+C's lock (`rideLocked`) —
   *  both ride a moving body and both want the same two properties (the sim's clock, and a
   *  spring rather than an ease); see followHeld's comment for why. They are mutually
   *  exclusive, so the one velocity between them is one follow's. */
  private easeFocusTo(c: readonly [number, number]): void {
  ...
  /**
   * Ctrl+C's camera lock, once a frame: keep the camera on the unit it was pressed over.
   *
   * It differs from the two follows beside it in both directions. It is a TOGGLE — nothing is
   * being held down, so it survives the keystroke — and it rides a UNIT rather than the
   * SELECTION, so the player goes on selecting, ordering and building while the camera stays
   * with the locked hero. That is the whole point of a lock, and it is why the id is stored
  ...
  private rideLocked(): void {
    ...
    const u = this.rts?.simWorld.units.get(this.cameraRide);
    if (!u) {
      this.cameraRide = null; // it died, or was consumed/loaded away — there is nothing to ride
      return;
    }
    this.easeFocusTo([u.x, u.y]);
  }

  /** Let go of a Ctrl+C lock (the player moved the camera themselves, or a script took it).
  ...
  private mountHud(): void {
    ...
    const driver: HudDriver = {
      ...
      focusSelected: (lock) => {
        ...
        this.releaseCameraRide(); // one follow at a time — the portrait's replaces Ctrl+C's
      ...
      followSelection: (on) => {
        ...
        if (on) {
          ...
          this.releaseCameraRide(); // …and Ctrl+C's, which rides the same way
          this.followVel[0] = this.followVel[1] = 0; // the double-tap already put us there, at rest
        }
      },
      // Ctrl+C: lock the camera onto the leading FRIENDLY unit of the selection and ride it.
      // A press with a lock already on is the release — the binding is one key, so the state
      // it toggles is read here rather than mirrored in the HUD.
      toggleCameraLock: () => {
        if (this.cameraRide !== null) {
          this.releaseCameraRide();
          return false;
        }
        const id = this.rts?.cameraLockTarget() ?? null;
        if (id === null) return false; // nothing selected, or what is selected is an enemy's
        this.cameraRide = id;
        this.cameraLock = false; // one follow at a time — this replaces the portrait's…
  ...
  private applyRaceCursor(): void {
    ...
    // The DOM stand-in for the pointer that is left, the scroll chevron, is sized in style.css
    // off this same number, so it cannot drift from the art we cut here.
    ...
    // Normal = the WC3 arrow everywhere. The two recoloured cursors are REAL cursors too, swapped
    // in through --ow3-overlay-cursor (updateReticle); the DOM stand-ins below hide it instead.
   ...
   *  the old canvas-drawn brackets. Null until the cursor sheet loads. */
  private reticleImg(colorKey: "green" | "yellow" | "red"): HTMLCanvasElement | null {
    const cached = this.reticleImgs.get(colorKey);
    ...
    this.reticleImgs.set(colorKey, c);
    return c;
  }
   ...
   *  "stays the same but pulsates green/yellow/red". Null until it loads. */
  private handImg(colorKey: "green" | "yellow" | "red"): HTMLCanvasElement | null {
    const cached = this.handImgs.get(colorKey);
    if (cached) return cached;
    const sheet = this.cursorSheet;
    if (!sheet) return null;
    ...
    ctx.putImageData(img, 0, 0);
    this.handImgs.set(colorKey, c);
    return c;
  }

  /** The `cursor:` value for the reticle or the tinted hand at the pulse's current step, cached.
   *
  ...
  private overlayCursor(kind: "reticle" | "hand", colorKey: "green" | "yellow" | "red"): string {
    ...
    const base = kind === "reticle" ? this.reticleImg(colorKey) : this.handImg(colorKey);
    if (!base) return "";
    const c = document.createElement("canvas");
    c.width = base.width;
    c.height = base.height;
    const ctx = c.getContext("2d")!;
    // A cosine through the cycle stands in for ease-in-out there and back.
    ctx.filter = `brightness(${1 + (0.85 * (1 - Math.cos((2 * Math.PI * frame) / PULSE_FRAMES))) / 2})`;
    ctx.drawImage(base, 0, 0);
    // The reticle is aimed from its CENTRE; the hand from the gauntlet's fingertip, as the plain one.
    const rule = kind === "reticle"
      ? cursorImageValue(c, c.width / 2, c.height / 2)
  ...
  stop(): void {
    ...
    this.hud?.hide();
    this.portraitViewer?.stop();
    this.hideCursorOverlay(); // restore the plain WC3 cursor
  ...
  private midPan(mx: number, my: number): void {
    this.releaseCameraRide(); // dragging the map by hand is taking the camera back (Ctrl+C)
   ...
   *  only, constant size) — green friendly / yellow neutral / red enemy. Both are real
   *  `cursor:` images (overlayCursor), swapped in through --ow3-overlay-cursor under the
   *  `armed-on` (screen-wide) or `reticle-on` (map-only) class.
   *
   *  `clientX`/`clientY` are VIEWPORT coords, for the carried item, which is still a DOM
   *  stand-in fixed to the body — it has to be free to follow the cursor out over the HUD and
   *  the letterbox. Feeding it canvas-space coords instead is what made the game feel broken
   *  windowed: the old DOM reticle drew itself a whole letterbox bar away from the real pointer,
   *  so you aimed with a cursor that was lying to you, and every click landed off. */
  private updateReticle(clientX: number, clientY: number): void {
    ...
    if (carrySlot >= 0) return this.hideCursorOverlay();
    ...
    // Written only when it changes: this runs every frame, and a custom property on <body> is
    // inherited by the whole document, so every write is a style recalc of all of it. It
    // changes at the pulse's own step rate, not the frame rate.
    if (rule !== this.overlayCursorRule) {
