...
const slot = (over = {}) => ({
  ...
  damage: 20, dice: 1, sides: 1, cooldown: 2.05, range: 100, rangeBuffer: 250, damagePoint: 0.1, backswing: 0.1,
...
console.log("\na map's own units can retune the clang (ucs1/ucs2)");
{
  ...
  const baseDef = () => ({
    ...
    animProps: [], attachAnimProps: [], attachLinkProps: [], upgradesUsed: [], tint: [1, 1, 1], targType: "ground",
