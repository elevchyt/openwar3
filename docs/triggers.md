...
| 7.15 | **Trigger threads — waits** (`TriggerSleepAction`/`PolledWait`): trigger actions run on a suspendable thread; `TriggerExecute`/`ConditionalTriggerExecute`/`ExecuteFunc` run INLINE on the caller's thread and **detach at a wait** | ✅ done (live) | §7.15 headless (a wait suspends + resumes on game time, **event responses survive it**, `PolledWait` through the real blizzard.j, 0s-wait can't hang, wait-in-condition abandoned, a wait in an executed trigger does NOT hold up its caller); ExtremeCandyWar + WarChasers park + resume real threads (screenshots) |
...
| 7.27 | **Destructibles — a gate OPENS by DYING** (issue #85). Reported as "gate doodads have no colliders", and there were two faults under it. (1) A destructible's pathing footprint **turns with the doodad**: both gates share one 20×4 `Gate1Path.tga` and are told apart by *facing* alone, so an unrotated stamp laid a vertical gate's wall **across** its corridor instead of through it. WC3's textures are authored in the World Editor's default facing, **270°** — verified against `DestructableData.slk`: applied to all 76 destructibles whose name declares HORIZONTAL/VERTICAL it gives the matching shape, 0 mismatches. (2) A `.doo` record whose `0x2` flag is clear is **not** "the mapmaker wanted a hole": it is the placeholder the editor leaves when it moves a doodad into `war3map.j` (which is what a `gg_dest_*` is), and skipping it left WarChasers' twenty gates drawn but walk-through. Then the natives, because a gate has no "open" state — `ModifyGateBJ` is `KillDestructable` + `"death alternate"`, and CLOSE is `DestructableRestoreLife` — so the collider follows LIFE, swapping to the `pathTexDeath` that blocks the two posts and leaves the middle walkable. `CreateDestructable` **adopts** the record already standing there rather than stacking a second gate on it | ✅ done (live) | `pnpm sim:test` §doodad-pathing (the 270° rest pose across all four facings incl. the editor's 266°/355° slop; rotation is a real rotation, not a transpose; a 20×4 at facing 270 blocks 20×4 and at facing 0 blocks 4×20; a placement with no facing is never turned; `life` is a PERCENT of the type's HP, so a record placed at 0 starts dead and one at 40 is a damaged tree; `CreateDestructable` adopts by position). WarChasers **live**: all **31** of its script-created records adopt a `.doo` record (`gg_dest_LTg1_0061` → doo #62), the gates' colliders sit exactly across the corridors `war3map.wpm` leaves open, and `ModifyGateBJ(OPEN)` drops the collider + swings the portcullis while CLOSE puts both back (screenshots). Invulnerability does **not** block `KillDestructable` — WarChasers marks all eighteen gates invulnerable and still opens them, so respecting the flag would weld the map shut at the first door |
| 7.28 | **Selection events — the map can tell what you CLICKED** (`EVENT_PLAYER_UNIT_SELECTED` / `_DESELECTED`, 24/25 player + 57/58 unit). Nothing raised them, so a whole genre of custom map — the hero picker, the sign you click to vote a mode, the shop that describes what you pointed at — had no input at all. Two rules separate this family from every other player-unit event, and a picker needs both. (a) The registration's player is the one who **SELECTED**, not the subject's owner: Extreme Candy War registers for players 0–4/6–10 and stands its costumes on slots **5 and 11**, which nobody plays, then reads `GetOwningPlayer(GetTriggerUnit())` itself — match on the owner the way the other 30-odd events do and it never fires for anyone. (b) A **double-click REBUILDS the selection** (clear + select afresh), which is what raises a *second* SELECTED on a unit you were already holding — the whole "click once for the description, twice to take it" idiom. A plain re-click raises nothing. Engine side: the RTS **diffs** its own selection each tick rather than raising at each mutation site, because a selection changes down a dozen routes (click, drag box, control group, hero bar, the script's own `SelectUnit`, a unit dying or walking into a mine) and every one must count; the re-selection pulse is the one thing a diff cannot see, so the click path records it | ✅ done (live) | `pnpm jass:test` §selection (the map's own picker shape through the real `TriggerRegisterPlayerSelectionEventBJ`: first selection previews and does not hand over, the deselect+select pair takes the hero, it is filed under the **clicker's** slot via `GetConvertedPlayerId`, a player who has picked cannot pick twice, an unregistered player raises nothing). ExtremeCandyWar **live**: double-clicking a costume gives you "Harp Damslayer" at the Horde spawn with his items and command card (screenshots) |
| — | **A Neutral Passive unit is not a static one** (same report). "Neutral Passive" is a SLOT; we read it as "never moves" and gave every such unit `speed 0` **and** never drew it from the sim, so Extreme Candy War's whole intro cast — pre-placed on that slot and moved by `IssuePointOrderLocBJ` from the cinematic trigger — accepted its orders and stood perfectly still. The split is BUILDING vs MOBILE: a shop/fountain/tavern keeps its map-placed Z and the viewer draws it, a critter or a cinematic extra gets its real speed from unit data and is drawn like any other unit. A **destructible** is viewer-drawn too (`Entry.borrowedBody`) — its Z is the doodad's, so driving one floats the crate | ✅ done (live) | `pnpm sim:test` unregressed; Echo Isles: shops/critters/destructibles all still drawn exactly where the map put them (dz 0); Candy: the intro's bats and villagers cover real ground (screenshots) |
| — | **A map's own imported sounds** (same report). `war3mapImported\…` is a path inside the `.w3x`, and the SoundBoard only ever looked in the install — so every imported clip a custom map plays was silent (Candy's intro music, its bats, wolves, taunts: 27 WAVs). The map archive is layered over the install for the match's lifetime, and comes back off at the end | ✅ done (live) | Candy **live**: `war3mapImported\HalloweenMusic.wav` decodes to 14.97 s stereo — matching the script's own `SetSoundDuration(…, 14968)` — and plays |
...
| 7.2f | **The map's own TECH TREE** — `war3map.w3u`/`.w3t`/`.w3a`/`.w3q` finally reach the tech GRAPH, not just the unit/item/ability/upgrade registries. Reported as *"most of the buildings show no actions, and the same for item buildings"* on **WTii's Unit Tester**. Everything a building OFFERS is a tech field — `Trains`, `Sellunits`, `Sellitems`, `Makeitems`, `Researches`, `Builds`, `Upgrade`, `Requires` — and **none of them is in an SLK**: `UnitMetaData.slk` says `slk = Profile` for every one, i.e. the per-race `*UnitFunc.txt` a map OVERRIDES. `TechRegistry` was built once from the install and never merged with the map's object data, so every type a map declares answered "trains nothing, sells nothing" and `buildCommandCard` had **nothing to draw**. Not a rare corner: a map that gives you a building to click is *made* of these fields — the tester is ~105 pre-placed Human Farms (`hhou`) whose entire content is a `Sellunits` list. The quieter half is the same bug from the other side: a CUSTOM id is in the install graph nowhere at all, so even a Tavern-based shop (`ntav`→`n001`) lost the base type's own wares, because a clone inherits nothing it isn't given. Fixed with the per-map overlay the other four registries already had (`applyMapTechData`), plus the three hard-coded tables the same map walked into: `usma`/`usrg`/`usst` + `isto`/`istr`/`isst` (a ware's **shelf**) had no setters; a **worker was a list of five ids** — WC3 has no such list, a unit gathers because it CARRIES `Ahar`/`Aaha`/`Ahrl`/`Awha`, so the map's own builders had no worker state and hence **no Build button**; and a **depot was a list of thirteen ids** — likewise no such list, a building takes deposits because it carries **`Artn`** ("Return Resources"), whose two Data columns ARE "accepts gold"/"accepts lumber", so the map's own Town Hall took none | ✅ done (live) | `pnpm sim:test` §map-tech (built w3u/w3q blobs, so it runs with no install: a Farm-based shop gets the map's `Sellunits`; a clone with no tech override INHERITS its base's wares and one that names its own REPLACES them; `Requirescount` GROWS the tier ladder; an ORIGINAL-table override reaches the base id **at every tier it clears** — which is how the tester's Tavern sells heroes with no altar standing; a pure stat/art override installs NO node; an upgrade's `greq` is tiered by LEVEL off `levelOrVariation`, verified against the tester's own `.w3q`; `clearCustom` re-indexes so one map's tier chain cannot answer for the next map's; a corrupt blob is non-fatal) + §map-data (the worker and depot rules, incl. the three real `Artn` rows and the TownHall gold stand-in). Live: the tester's "Orc Units" shop draws its twelve orc wares with stock badges and really queues a Peon; "Shop 8" draws its eleven potions; "Human Special Heroes 1" its eleven heroes; "Human Tech 2" its ten researches; its own builder ("All Round Nice Guy", a Peasant carrying `Ahar`) has a Build button whose page is the map's own eight structures; and its own `htow`-based Town Hall (`h02X`) takes gold — that builder ran eight real mine trips into it, +80 gold. Echo Isles unregressed — hall, Peasant build page, Tavern wares, and a live harvest round trip (500 → 520); every hall of all four races reads gold+lumber, the Lumber Mill / War Mill / Graveyard lumber-only, a Barracks neither (screenshots) |
| 7.2g | **Three rules the map decides and we were deciding for it** — reported against the real client on WTii's Unit Tester once 7.2f made its buildings clickable. (1) **Only a building that TRAINS takes a rally point.** `Trains` and `Sellunits` are different fields and only the first rallies — there is no Set Rally Point on a Tavern, a Mercenary Camp, a Goblin Laboratory or a Shipyard, and the only buildings the real client rallies on the tester are its *altars*. Ours read `trains ‖ sellunits`, so forty-odd shops wore a rally button, and it **cost the 12-ware shops a ware**: reserving (3,1) left the twelfth nowhere to go, which is why the Wyvern Rider was missing from "Orc Units". The other half is not cosmetic — a hired unit was being walked to a rally point it should never have had, and this map **teleports** every bought unit into an arena the instant it enters the shop's region, so the unit arrived and then walked all the way back out. (2) **A map can CLEAR a building's ground texture** (`uubs` → unitUI `uberSplat`); an emptied field is a real value, "this building scars no ground". The tester empties it on **all 111** of its buildings — they are Human Farms wearing other models — and every one was drawing the Farm's `HSMA` foundation ring on the grass. (3) **The creep camps**: the whole map is enter-region driven (48 `TriggerRegisterEnterRectSimple`), and buying a marker unit at a "Creep - …" shop is answered by a trigger that explodes the marker and builds the camp at its own rects. That needed nothing new — a unit CREATED inside a watched rect already counts as entering it — but it was dead until 7.2f made the shop's card exist, and it is now the sharpest end-to-end proof the trigger engine has | ✅ done (live) | `pnpm sim:test` §map-tech (`producesUnits` is `Trains` only: the map's altar rallies, its unit shop / item shop / stock Tavern do not, a Town Hall does; and the `uubs` rule: cleared reads empty, a clone that says nothing keeps the base's, one naming another decal gets that one, the base type is untouched) — the rally rule now lives in ONE place (`TechRegistry.producesUnits`) because two callers derive it. Live on the tester: the altars are the only buildings wearing a rally flag; "Orc Units" draws **all twelve** wares (`owyv` back at 3,2); a Peon bought there is teleported to `gg_rct_Created_Units_Spawn_Here` and **stays** there, idle; buying "Creep - Green (Average) - Gnolls" explodes the marker and stands three Neutral-Aggressive gnolls in `gg_rct_Creep_Green_2a/b/c`; and the map's clone of the Farm stands on clean grass beside a stock Farm still wearing its ring (screenshots) |
| 7.2h | **Three follow-ups from playing 7.2g.** (1) **The map's object data must load BEFORE the map's geometry**, not at the top of `startCustom` hundreds of lines later. `stampMapPathing` paints every pre-placed building's ubersplat (and reads its pathing footprint) against the registry as it stands *then* — so the tester's fourteen **mercenary camps kept their decals**, because the map clears `uubs` on them through the ORIGINAL table and the stamp had already run off the un-overridden row. Its custom `h0xx` shops only looked right by accident: a custom id resolves to nothing before the overlay lands, so nothing was painted at all. Moved to the moment the archive is opened — which also means a MELEE map with object data finally gets it, since the `startCustom` call meant melee read its own `.w3u` nowhere. (2) **A building is BORN with no rally point.** WC3 sets no default: a fresh Barracks has no flag and the unit it trains walks clear of the door and stops. Ours defaulted to a point 200 units south, so every unit any building ever produced marched off it — a hero bought at an altar ran away from the altar. `RallyKind` gains **`"none"`**, which is the birth state; no flag is drawn and `applyRally` returns. (3) **`Player(PLAYER_NEUTRAL_AGGRESSIVE)` is the CREEP slot, not a thirteenth player.** A unit the script creates on it was coming out owned by a live player — coloured, on a team of its own, allied to nobody and attacking no one. It now takes the same owner/team pair (`-1`/`-1`) and the same guard AI `trySeed` gives a map-PLACED creep, so both are one thing to the sim; slots 13–15 take the neutral-PASSIVE pair for the shops and critters a script makes | ✅ done (live) | Live on the tester: the mercenary-camp row stands on clean grass; a Paladin hired at an altar is idle where the map's own trigger teleported it (the altar's `rallyKind` is `"none"` and it draws no flag); the Gnoll camp comes up `owner -1 / team -1 / isCreep` |
| 7.2i | **A tower defence's wave clock** — `TriggerRegisterPlayerStateEvent`, i.e. **`EVENT_PLAYER_STATE_LIMIT`**, the player-scoped twin of 7.17's unit-state threshold. Reported as *"the game locks after the first wave"* on **(8)AzureTowerDefense**, and it is not a hang at all: a round-based map does not count its creeps, it asks the engine how much **food** the monster player is using and calls the round over when that reaches zero. Azure's entire round loop is one registration — `TriggerRegisterPlayerStateEvent( gg_trg_Monster_Spawning, Player(10), PLAYER_STATE_RESOURCE_FOOD_USED, EQUAL, 0.00 )` — whose action counts the round up, runs the 30s countdown and `TriggerExecute`s the spawn. Nothing else ever runs that trigger after the one-shot 60s opening timer, so with the native missing the registration was dropped on the floor and the map simply stopped the moment wave 1 was cleared. Implemented exactly like its twin, and for the same reason — nothing in the sim raises "this player's food changed" — so it is **polled** per tick and fires on the **rising edge**, seeded at registration (`playerStateHolds`). The seed matters more here than for a unit: "food used == 0" is true at map init and true for the whole gap between waves, so a level-triggered version fires once before anything has spawned and then again every tick of the countdown it just started. (Azure guards that case itself with a `udg_Spawner` flag — the tell that Blizzard's own designers expected an edge.) `getPlayerState` is the same hook `GetPlayerState` reads, so the poll and the script cannot disagree about the number | ✅ done | `pnpm jass:test` §player-state, through the real `common.j`/`blizzard.j` and shaped like Azure's own loop: registering while the comparison already holds does **not** fire; a wave in the field and a partly-killed wave do not; the last creep dying starts round 1 with `GetTriggerPlayer` = the monster player; the countdown's own ticks do not re-run it; and wave 2 crosses again — the part that was hanging. A gold threshold that never crosses stays quiet, so the registration's `playerstate` is really being read. Azure's native census is now **0 missing** (every `common.j` native the map calls is implemented) |
| 7.2j | **`PLAYER_STATE_RESOURCE_HERO_TOKENS`, and with it the free first hero.** Reported as *"the first hero from the Altar of Summoning is free, and this is not a melee map"* on **(8)AzureTowerDefense**. WC3's "first hero is free" is not a rule about heroes at all — it is a **resource**, and one only the melee opening hands out. Blizzard.j spends five lines on it, every one of them inside `MeleeStartingUnits*` and every one a SET rather than an add: `SetPlayerState(whichPlayer, PLAYER_STATE_RESOURCE_HERO_TOKENS, bj_MELEE_STARTING_HERO_TOKENS)`, right under the comment *"give them a 'free hero' token"*. A custom map runs none of that, so its players have no tokens and every hero costs full price. We modelled it as a boolean that defaulted to **yes**, so hero #1 was free on every map ever loaded. Now a real per-player count that starts at **0**: `Authority.heroTokens`, granted through the state the script actually writes (state **3**, which both `SetPlayerState` and `GetPlayerState` were silently dropping) — so the scripted melee path grants it by running Blizzard's own line, and `startMeleeFallback`, which stands in for that script, grants it explicitly alongside the starting gold and lumber | ✅ done (live) | `pnpm sim:test` §build-orders: a player starts with none; the melee grant is idempotent (it is a SET); spending leaves none and cannot go negative; a cancelled hero hands the token back; the grant is per player. Live: on **Azure** `hasFreeHero` is false and the Altar of Summoning's three heroes cost their real 150 / 225 / 300 gold; on **Echo Isles** it is still true and `GetPlayerState(p, 3)` reads **1**, granted by Blizzard.j's own call — the authority and the script agree |
...
| `natives/destructables.ts` | **destructibles** (7.27): the `destructable` handle over the map's own `war3map.doo` records — life (`Kill`/`DestructableRestoreLife`/`Set`/`GetLife`/`MaxLife`), animation, `Remove`/`Show`, identity, `EnumDestructablesInRect`. `CreateDestructable(Z)` **adopts** the record already standing at those coordinates rather than making a second one. There is no gate native: `ModifyGateBJ` is blizzard.j riding on life + animation, so implementing those lights up the whole GUI gate vocabulary |
...
- **A resource DEPOT is not a list of thirteen ids either**, and it is the same ability trick one step along:
  **`Artn`** ("Return Resources"), whose two Data columns (`Rtn1`/`Rtn2` in `AbilityMetaData.slk`) **are** the two
  flags the sim already keeps. The game ships exactly three aliases of it and the whole rule is in that table:
  `Argl` "Return (Gold & Lumber)" (1,1) on the human and orc halls, `Arlm` "Return (Lumber)" (0,1) on the Lumber Mill,
  the **War Mill**, the **Graveyard**, the Necropolis chain and the Tree chain, and `Argd` "Return (Gold)" (1,0) which
  nothing stock carries. So WTii's Unit Tester's own `htow`-based hall (`h02X`) now takes deposits — it inherits
  `Argl` the way it inherits its model — and two holes in the STOCK data close with it: `ofor` and `ugrv` were not in
  the old id set, so orc and undead lumber had to be walked all the way back to the hall. **One deliberate departure
  from the data:** a `TownHall`-classified building takes GOLD even when its `Artn` row says lumber-only, because the
  Necropolis and Tree chains carry only `Arlm` — in the real game neither race's gold ever leaves the mine, and we
  model the night elf's version but not the undead's ("we do not model a Haunted Gold Mine", `world.ts`), so our
  acolytes shuttle it and need somewhere to put it. Drop that clause when the Haunted Gold Mine lands.
...
### …and three rules the MAP decides that we were deciding for it (7.2g)

Reported against the real client once 7.2f made the tester's buildings clickable:

- **Only a building that TRAINS takes a rally point.** `TechRegistry.producesUnits` is `Trains` and nothing else —
  `Sellunits` is a different field and a different thing. WC3 puts no Set Rally Point on a Tavern, a Mercenary Camp, a
  Goblin Laboratory or a Shipyard, and on the tester the only buildings the real client rallies are its altars. One
  rule in one place because two callers derive `BuildingState.producesUnits` from it (birth and upgrade) and they must
  never disagree. Two consequences beyond the button: a 12-ware shop fills the 4×3 card exactly, and reserving (3,1)
  for a rally cost it a ware; and a hired unit is no longer WALKED anywhere — `applyRally` is now gated on
  `SimWorld.acceptsRally`, which matters because this map teleports every bought unit into an arena the moment it
  enters the shop's region, and a unit still carrying a move order walked straight back out.
- **A map can CLEAR a building's ground texture** — `uubs` (unitUI `uberSplat`). An emptied field is a value, not an
  absence: "this building scars no ground". The tester empties it on all 111 of its buildings and every one was
  drawing the Human Farm's `HSMA` foundation ring on the grass underneath.
- **The creep camps needed nothing new, and that is the point.** The whole map is enter-region driven — 48
  `TriggerRegisterEnterRectSimple` — and a "Creep - …" shop works by selling a marker unit that is BORN inside
  `gg_rct_Kill_Trained_Creep_Units`: one trigger explodes it, another matches its type and builds the real camp at its
  own three rects with `CreateNUnitsAtLoc` under `PLAYER_NEUTRAL_AGGRESSIVE`. `pumpRegions` already counts a unit
  created inside a watched rect as entering it (it is absent from the previous membership set), so the mechanic came
  alive the moment 7.2f gave the shop a card. It is now the sharpest end-to-end check the trigger engine has, and it
  is worth knowing WHY it was silent in testing before that: **`mapScene().mapScript = null` — the camera-freeing move
  in the `live-browser-testing` memory — switches off the entire trigger pump.** Freeze the camera through
  `scriptCam`/`cinematic` instead when what you are testing is a trigger.
...
- **Thread-transparent natives** — `TriggerExecute` / `ConditionalTriggerExecute` / `ExecuteFunc` run more JASS *on the
  ...
  They run INLINE — until they hit a wait, at which point **the rest of them becomes a thread of its own and the caller
  carries straight on**. `main()` and `config()` run on threads too.

  That last rule is the intuitive reading's opposite, and it took three sources to settle. It is **tested**: the Hive
  thread on exactly this question ([TriggerSleepAction inside TriggerEvaluate/TriggerExecute][tsa]) reports the
  experiment — a sleep inside `TriggerEvaluate` *kills* the thread and it never resumes, a sleep inside `TriggerExecute`
  *returns control to the calling thread*, and `TriggerExecuteWait` behaves the same as `TriggerExecute`. blizzard.j's
  own TRIGGER QUEUE cannot work any other way either: `QueuedTriggerAttemptExec` runs a queued trigger with
  `TriggerExecuteBJ`, then **arms a timeout timer** and waits for the trigger to call `QueuedTriggerDoneBJ` on its way
  out — both pointless if the call had blocked until the trigger finished. And it is what map scripts assume: Extreme
  Candy War's hero picker fires a `TriggerExecute` at a cleanup trigger that `PolledWait`s 7 s and then hands you the
  hero on the next line, so blocking meant a seven-second stare at the costume you had just double-clicked.

  [tsa]: https://www.hiveworkshop.com/threads/triggersleepaction-inside-triggerevaluate-triggerexecute.275707/
...
not spun; and a `Wait` in an executed trigger does **not** hold up the caller — init finishes, the trigger parks, and
its unit spawns when the wait runs out. Verified **live**: ExtremeCandyWar parks its intro-cinematic + income threads from t=0 and resumes them on
...
- **The events** — the sim records each one only when the script listens (`captureSpells` /
  ...
  - **`EVENT_UNIT_STATE_LIMIT`** (59, via `TriggerRegisterUnitStateEvent`) — the one event nothing
    ...
    **rising edge**: "life drops below 100" fires once per crossing, not every tick it sits below.
    ...
    fires. Its player-scoped twin is **`EVENT_PLAYER_STATE_LIMIT`** — see 7.2i.
...
### The subtitle panel: two ways to read the FDF wrong

Both were only visible next to a screenshot of the real client, and both are the same mistake —
overriding the file instead of reading it.

- **The speaker's name is not the player's colour.** We painted it
  `PLAYER_COLORS[scene.playerColor]`, so Maiev's every line came up blue. `SetCinematicScene`
  *does* take a `playercolor` — that's where the idea came from — but it is not what tints this
  line. `CinematicSpeakerText` INHERITS `EscMenuTitleTextTemplate` and overrides no `FontColor`,
  so it wears the template's `FontColor 0.99 0.827 0.0705` — the gold every transmission in the
  game titles its speaker in. (`CinematicDialogueText` inherits the same template and *does*
  override it, to white. The file distinguishes them; we shouldn't have.)
- **An appended prop does not override an inherited one.** The subtitle box is three lines tall
  (the file gives it a Width and no Height), so it needs `FontJustificationV JUSTIFYTOP` to grow
  DOWNWARD from the speaker's line — and it had one. But `firstProp` takes the FIRST match and
  INHERITS had already merged the template's `JUSTIFYMIDDLE` into the frame's props, so the
  appended JUSTIFYTOP was never read and a one-line message floated a full line below the name,
...
**A fourth surface joined them later, and it is the one a player notices**: the top-right stack —
the leaderboard, the multiboard and the countdown windows (`updateScriptUi`). Those are interface
too, and WC3 has no scoreboard hanging over a cutscene; ours stayed up, so Extreme Candy War's
intro played under its own kill-count board. Keyed on `interfaceShown` alone rather than the pair,
for the same reason the cursor and the console panels are: `EnableUserUI(false)` is the momentary
blackout blizzard.j flicks around each cinematic FADE, and a board must not blink for it.
...
player and stops there; the skip itself is written out longhand in every campaign chapter.
NightElfX01:
...
Created **disabled**, enabled by the intro trigger at its own comment — `// CINEMATIC BEGINS -
Cinematic Can Now Be Skipped` — and disabled again by the cleanup, so the map owns the window in
which ESC does anything. Its action then fades out, runs the cleanup trigger, leaves cinematic
mode and starts the gameplay trigger; its first line is `DisableTrigger( GetTriggeringTrigger() )`,
so a second tap during the fade cannot run the teardown twice. `Interpreter.firePlayerEvent`
matches the registration's player and event enum, and `fireTrigger` already honours `enabled`.

The raiser is `render/mapViewer.ts`, gated on **`ShowInterface(false)`** — cinematic mode is what
...
- **A unit whose model is BLANK does not exist for us at all.** WC3's convention for an invisible unit is a model field
  of `" .mdl"` — a single space — and a map uses it for every dummy: Extreme Candy War has four (`Dummy Caster`,
  `Dummy Mage Unit`, and the two `Dummy Cinematic Vision` units that give the players sight of its intro). We build a
  body FIRST and the sim unit second, both for pre-placed units (`trySeed` walks the viewer's widgets, and there is no
  widget for a model that does not resolve) and for script-created ones (`spawnUnit` returns null when the model fails
  to load), so a dummy is simply missing: its vision is missing, and `RemoveUnit` on it does nothing. Needs a
  "unit with no body" path on both routes. Found while fixing 7.29 — before it, the dummies were *visible*, which is
...
- **A card with more than twelve buttons still drops the overflow** (`layoutCard`) — but only one case is left. The
  rally half is fixed (7.2g: a `Sellunits` shop has no rally point, so twelve wares fit the 4×3 exactly, which is what
  the real client shows). What remains is an ITEM shop with twelve wares: `Anei`'s Select User is pinned at (3,2) by
  its own data, so "Shop 8" still loses its twelfth. Blizzard's own shops stop at eight wares, so the stock data never
  reaches this; which button the real client sacrifices there is unverified, so nothing is invented.
...
- **Effect natives still missing.** 7.7 + 7.13 cover resources, unit-state and the unit-mutation set; 7.16 the group +
  ...
  (`TriggerRegisterPlayerChatEvent` — 3) and **lightning** (2). Each is a
...
- **Events still missing:** `EVENT_PLAYER_UNIT_SUMMON` (the sim has the summon channel — same "born in the renderer"
  shape as TRAIN_FINISH), `..._RESEARCH_*` / `..._UPGRADE_*` (no upgrade system yet), `_SELL`/`_SELL_ITEM`
  (no shops — above). *(`..._SELECTED`/`_DESELECTED` landed — 7.28; `EVENT_PLAYER_STATE_LIMIT` — 7.2i.)*
