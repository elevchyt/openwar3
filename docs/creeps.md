...
| **Lightning Shield** wants the wearer TOUCHING two others (three within `Area1` 160, edge to edge, and a BUILDING counts — the Ancient of War). **Purge** prefers a summon. **Hurl Boulder** prefers the hero. **Slam** wants three. | 176; warcraft-gym | `CAST_RULES` in casting.ts |
| **The Lightning Shield creep trick**: the Renegade Wizard never shields a unit its OWN side stands beside, itself included — the shield would burn them. So it must be "attacked from afar" (melee alone takes "finesse": the attacker is touching the wizard), and the shielded unit is then walked into the camp. A shield's kills belong to whoever OWNS the shield — the wizard's pay no experience or bounty, a player's Shaman's do — **and stay that way after the caster dies**. | 176; maintainer | `touching`/`spares` in casting.ts, `burnsOwn`; `shieldCasters`/`killerUnit` in world.ts |
...
| A creep dozes off only once its **camp is quiet**: no camp-mate attacking or casting (at any enemy, not only one it could fight itself), nothing hostile with its attack on the camp, and no member struck for `CREEP_SLEEP_CALM` (3 s). A camp whose fight has ended still sleeps straight away after that. The 3 s is ours (the maintainer's) — no file states a sleep delay. | maintainer | `CREEP_SLEEP_CALM`, `campQuiet`, `SimUnit.struckAt`, tickCreep |
...
| A creep that can **Hide never sleeps**: at night it melds instead and keeps watch, and anything that enters its aggro range springs the ambush — out of hiding, onto the target, and a shout that wakes the whole camp. | maintainer | `hidesAtNight`, `creepMelded`, `creepAmbush`, tickCreep |
