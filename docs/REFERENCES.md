...
| [Hive: "Spell/Ability Damage Types and what they mean" (316271)](https://www.hiveworkshop.com/threads/spell-ability-damage-types-and-what-they-mean.316271/) | **The DAMAGE TYPE of every damaging ability**, tested by Bribe and Tasyen, plus the attack-type × damage-type grid for who is hit: Magic/Fire/Cold/Force/… never reach a spell-immune unit, Normal and Enhanced reach it but not an ethereal one, Universal reaches both. The type cannot be changed in the Object Editor, so it is a fact about the base `code` | Drives `FIELD_PIERCES_SPELL_IMMUNITY` (src/sim/spells.ts). Pair it with Liquipedia's Spell Immunity page, which adds that an immune unit still SOAKS its share of a capped wave. hiveworkshop 403s plain fetchers: use a browser `User-Agent` |
| [Hive: "Help with RTS pathfinding" (352974)](https://www.hiveworkshop.com/threads/help-with-rts-pathfinding.352974/) | **How WC3's own pathfinder behaves**, from people who tested it and reimplemented it. bear_369 ([post 3614130](https://www.hiveworkshop.com/threads/help-with-rts-pathfinding.352974/post-3614130)): a blocked unit **repairs its queued path** — drops the nodes no longer traversable, searches to the first that still is, splices — and units never route *around* a crowd ("rather attempt to move through a shortest path blocked by bunch of units than through a long path with open space"). Dr Super Good: on a unit-unit collision "the engine simply makes both units stop and wait for a pathing update"; tile-based map, mini-moves per sim frame interpolated per render frame. Retera (Warsmash): WC3 **throttles** pathfinding — past ~100 units "they stand and stare… the purpose for that throttle is to avoid frame-dropping" | The OBSERVED parts (repair, stop-and-wait, throttle) are first-hand; the HPA*/fixed-size-regions structure in the same post is explicitly bear_369's *belief*, not RE — do not build on it as fact. hiveworkshop 403s plain fetchers: fetch with a browser `User-Agent`, and page 2 holds the key posts. Warsmash's own answer is a job queue at 7,500 iterations/tick with resumable jobs ([CPathfindingProcessor](https://github.com/Retera/WarsmashModEngine/blob/main/core/src/com/etheller/warsmash/viewer5/handlers/w3x/simulation/pathing/CPathfindingProcessor.java), [PR #31](https://github.com/Retera/WarsmashModEngine/pull/31), [issue #59](https://github.com/Retera/WarsmashModEngine/issues/59)) — study, don't lift |
